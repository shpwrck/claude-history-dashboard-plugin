...
- **Running unit:** container `chd-deploy-master_app_1`, podman compose project
  `chd-deploy-master`, image `ghcr.io/shpwrck/claude-history-dashboard:latest`,
  host port `5173`. Not managed by systemd (any `PODMAN_SYSTEMD_UNIT` label on
  the container is vestigial — `systemctl --user` reports the unit `not-found`).
...
- ghcr.io/shpwrck/claude-history-dashboard:latest (published by .github/workflows/docker-publish.yml on every master push) -> the image the container runs
...
whatever CI last published to `:latest`; wait for `docker-publish.yml` to finish
after a master merge before refreshing.
...
- **Confirm the intended build is live:** `podman inspect chd-deploy-master_app_1 --format '{{range .Config.Env}}{{println .}}{{end}}' | grep '^GIT_SHA='` should equal the target master commit; `/api/recommendations.json` should return a JSON array.
...
- **Rollback:** every master push also publishes `ghcr.io/shpwrck/claude-history-dashboard:sha-<short>`. Pin the previous tag (edit the image ref or `podman tag` it to `:latest`) and `up -d` to roll back to a known-good build.

## Decision log

### Decisions

- **Published-image pull path, not `--build`.** The standing instance mirrors the
  CI-published `:latest` so it matches what ships, and refresh is a fast pull +
  recreate rather than a local rebuild. `npm run deploy`/`--build` is for
  verifying uncommitted local frontend changes, not this instance.
...
`docker-publish.yml` to publish `:latest` for the merge commit
(`gh run list --workflow=docker-publish.yml --branch master`); (2) **as the
user**, run `chdref` to pull + recreate with the LAN posture, or run the
agent-safe loopback command above if you are an agent; (3) verify `200` on
`/api/dashboard-status` and that `GIT_SHA` matches the merge commit; (4) if it
crash-loops, check `BIND_HOST` / insecure-flag propagation per Verify and
recover. To change the exposure posture, edit the `~/.bashrc` exports (or add
`DASHBOARD_USER`/`DASHBOARD_PASS`) and re-run `chdref` once — do not hand-edit the
container.
