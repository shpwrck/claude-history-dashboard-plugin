 ...
 *     non-portable one, and lets illustrative/manual bypass; validateNpmRunScripts
 *     accepts a present script and rejects an absent one.
 *  2. Gate: a source scan over every detector — any file that embeds a
 *     non-portable reference (claude-team CLI, a /skill invocation) in a snippet
...
describe('validateFixSnippet', () => {
  ...
  it('rejects a validated snippet invoking the claude-team CLI', () => {
    expect(
      validateFixSnippet({ snippet: 'claude-team redispatch --team x --only-unread' }).length
    ).toBeGreaterThan(0);
  });
  it('lets a manual or illustrative fix bypass the gate', () => {
    ...
    expect(validateFixSnippet({ fixKind: 'illustrative', snippet: 'npm run -s typecheck' })).toEqual([]);
  });
});
...
// files — they reference `claude-team` in their own docs/patterns, so the gate
...
describe('detector fix-snippet portability gate (#1101)', () => {
  it('every detector embedding a non-portable reference marks its fix non-validated', () => {
    ...
    for (const file of detectorSourceFiles()) {
      ...
      const hasNonPortable = NON_PORTABLE_SNIPPET_PATTERNS.some((p) => p.pattern.test(src));
