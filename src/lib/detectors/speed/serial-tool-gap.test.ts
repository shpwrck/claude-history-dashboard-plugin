...
describe('speed.serial-tool-gap (#1753, epic #1910)', () => {
  ...
  it('fires with a clamped lower-bound band and per-inference time credit', () => {
    const entries = [
      ...read(0, '/src/a.ts', 'a'),
      ...read(4, '/src/b.ts', 'b'),
      ...read(8, '/src/c.ts', 'c'),
    ...
    expect(rec?.detail).toContain('Avoidable round-trips: 3-3');
    expect(rec?.detail).toContain('lower bound');
    expect(rec?.detail).toContain('includes tool-execution time');
    // 3 round-trips x 2s floor = 6s ≈ 0 min (rounded), still a defined number.
    expect(typeof rec?.estTimeReclaimedMin).toBe('number');
