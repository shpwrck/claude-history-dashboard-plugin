...
function pluginFiles(root) {
	...
	const walk = (directory) => {
		for (const entry of readdirSync(directory, { withFileTypes: true }).sort((left, right) => left.name.localeCompare(right.name))) {
			...
			if (metadata.isSymbolicLink()) fail$4(`Sidekick plugin snapshot refuses symlink ${path}`);
			if (metadata.isDirectory()) {
				walk(path);
				continue;
			}
			if (!metadata.isFile()) fail$4(`Sidekick plugin snapshot refuses special file ${path}`);
			...
			if (files.length > MAX_PLUGIN_FILES || totalBytes > MAX_PLUGIN_BYTES) fail$4("Sidekick plugin snapshot exceeds its fixed bounds");
		}
	};
	for (const relativeRoot of SIDEKICK_SNAPSHOT_ROOTS) {
		const directory = join(root, relativeRoot);
		if (!existsSync(directory)) fail$4(`Sidekick plugin snapshot is missing ${relativeRoot}`);
		const metadata = lstatSync(directory);
		if (!metadata.isDirectory() || metadata.isSymbolicLink()) fail$4(`Sidekick plugin snapshot root is not a real directory: ${directory}`);
		walk(directory);
	}
	return files;
}
...
function assertGate2702SandboxPreDispatch({ preDispatch, registration, worktreeIdentity }) {
	...
	if (!Array.isArray(sandbox.tools) || sandbox.tools.length !== 3 || sandbox.tools.some((tool) => !tool || typeof tool !== "object" || Array.isArray(tool) || ![
		...
		"socat",
		"rg"
	].includes(tool.name) || !isAbsolute(tool.command ?? "") || !isAbsolute(tool.resolved ?? "") || tool.version !== null && typeof tool.version !== "string") || new Set(sandbox.tools.map((tool) => tool.name)).size !== 3) fail$4("pre-dispatch sandbox tools are not the fixed runtime set");
	...
	if (hostHome === resolve("/") || isolatedHome !== expectedIsolatedHome || !sameValue$4(sandbox.policy.filesystem.denyRead, [hostHome]) || sandbox.policy.filesystem.allowRead.includes(hostHome) || !sameValue$4(sandbox.policy.filesystem.denyWrite, ["/tmp/claude", "/private/tmp/claude"]) || !sameValue$4(sandbox.allowedWriteRoots, uniqueSorted([
		registration.worktreePath,
		isolatedHome,
		worktreeIdentity.gitDirectory
...
function validateBehaviorContext(context, treatmentId) {
	...
	if (context === null || typeof context !== "object" || Array.isArray(context) || context.schemaVersion !== 2 || !validTimestamp(context.observedAt) || context.workerModelQualifiedId !== "claude-haiku-4-5-20251001" || context.sidekickModelQualifiedId !== (enabled ? "claude-sonnet-5" : null) || context.sidekickVersion !== (enabled ? "0.3.3" : null) || (enabled ? !SHA256_PATTERN$1.test(context.sidekickImplementationDigest ?? "") : context.sidekickImplementationDigest !== null) || !sameValue$3(context.sidekickActivation, enabled ? {
		pluginEnabled: true,
...
function validateWorkerLifecycle(artifacts, registration, snapshot, preflight, identity, anchor) {
	...
	try {
		...
		fail$3("worker dispatch does not rederive from its production inputs");
	}
	const workerDispatchValid = sameValue$3(sandbox.workerArgv, [
		sandbox.workerArgv[0],
		...WORKER_ARGV.slice(1),
		"--plugin-dir",
		sandbox.sidekickSnapshot.path
	]);
	if (preDispatch.registrationDigest !== registration.contentDigest || preDispatch.worktreeIdentityDigest !== identity.contentDigest || preDispatch.executionMode !== "production" || !workerDispatchValid || !sameValue$3(preDispatch.sidekickEnvironment, expectedEnvironment) || preDispatch.sidekickEnvironmentDigest !== valueDigest$3(expectedEnvironment) || resolve(preDispatch.cwd ?? "") !== resolve(registration.worktreePath) || preDispatch.promptDigest !== sha256Bytes$2(Buffer.from(workerPrompt$1(snapshot, registration), "utf8")) || !validTimestamp(preDispatch.startedAt) || terminal.preDispatchDigest !== preDispatch.contentDigest || typeof terminal.durationMs !== "number" || !Number.isFinite(terminal.durationMs) || terminal.durationMs < 0 || !validTimestamp(terminal.endedAt)) fail$3("worker dispatch does not rederive from its production inputs");
	let processReceipt = null;
...
function withReceiptDigest$1(receipt) {
	const withoutDigest = { ...receipt };
	delete withoutDigest.contentDigest;
	return {
		...withoutDigest,
		contentDigest: valueDigest$2(withoutDigest)
	};
}
function validateSandboxedWorkerDispatch(preDispatch, registration, worktreeIdentity) {
	...
	if (registration.executionMode === "production") {
		...
		if (sameValue$2(sandbox.workerArgv, expectedSandboxedArgv)) return sandbox;
		fail$2("worker origin provenance does not bind the fixed C5 invocation");
	}
	return sandbox;
}
function buildGate2702JudgeOriginProvenance({ registration, preDispatch, worktreeIdentity, classification, workerResultDigest, worktreeEvidence }) {
	const sandbox = validateSandboxedWorkerDispatch(preDispatch, registration, worktreeIdentity);
	const body = {
		...
		sandboxAllowedReadRootsDigest: valueDigest$2(sandbox.allowedReadRoots),
		sandboxAllowedWriteRootsDigest: valueDigest$2(sandbox.allowedWriteRoots),
	...
	if (!["production", "test"].includes(body.executionMode) || !SHA256_PATTERN.test(body.workerStdoutDigest ?? "") || !SHA256_PATTERN.test(body.workerResultDigest ?? "") || !SHA256_PATTERN.test(body.worktreeEvidenceDigest ?? "")) fail$2("worker origin provenance is not a production evidence chain");
	return withReceiptDigest$1(body);
}
function validateOriginProvenance(provenance, sourceArm, evidence, worktreeEvidence) {
	if (provenance === null || typeof provenance !== "object" || Array.isArray(provenance) || provenance.schemaVersion !== SCHEMA_VERSION || provenance.kind !== "Gate2702JudgeOriginProvenance" || !SHA256_PATTERN.test(provenance.contentDigest ?? "") || provenance.contentDigest !== receiptDigest$1(provenance)) fail$2(`${evidence.treatmentId} origin provenance receipt is absent or invalid`);
	if (!sameValue$2(provenance, buildGate2702JudgeOriginProvenance({
		...
		worktreeEvidence
	}))) fail$2(`${evidence.treatmentId} origin provenance does not bind its sandboxed sources`);
	return provenance;
}
...
function validateFrozenSource(artifacts, anchor) {
	...
	for (const treatmentId of TREATMENTS$1) {
		...
		if (preDispatch.treatmentId !== treatmentId || preDispatch.attempt !== selected.attempt || preDispatch.registrationDigest !== registration.contentDigest || normalizedPath(preDispatch.cwd) !== normalizedPath(registration.worktreePath) || preDispatch.promptDigest !== sha256Bytes$1(Buffer.from(workerPrompt(snapshot, registration), "utf8"))) fail$2(`${treatmentId} worker dispatch prompt or invocation is invalid`);
...
function validateFrozenInput(artifacts, anchor) {
	...
	if (input.pairSelectionDigest !== source.selection.contentDigest || input.subjectSnapshotDigest !== source.snapshot.contentDigest || input.task !== `${source.snapshot.title}\n\n${source.snapshot.body}` || !exactTreatmentKeys(input.armEvidence) || !exactTreatmentKeys(input.originProvenance) || !exactTreatmentKeys(input.objectiveChecks) || !exactTreatmentKeys(input.artifacts)) fail$2("frozen judge input does not rederive from the selected C5 pair");
	const frozenArms = {};
	for (const treatmentId of TREATMENTS$1) {
		...
		const originProvenance = validateOriginProvenance(evidence.originProvenance, arm, evidence, expectedWorktreeEvidence);
		if (input.armEvidence[treatmentId] !== evidence.contentDigest || input.originProvenance[treatmentId] !== originProvenance.contentDigest || input.artifacts[treatmentId] !== evidence.artifact || objective?.classificationDigest !== arm.classification.contentDigest || !sameValue$2(objective?.results, arm.classification.checkResults) || objective?.state !== objectiveState) fail$2(`${treatmentId} frozen judge evidence does not match its input`);
...
function expectedPayloads(input) {
	const forward = {
		...
		originProvenance: {
			A: input.originProvenance[TREATMENTS$1[0]],
			B: input.originProvenance[TREATMENTS$1[1]]
		}
	};
	return {
		forward,
...
var SCHEMA_VERSION, DEFINITION_REF$1, TREATMENTS$1, CHECK_IDS$1, JUDGE_MODEL$1, JUDGE_TIMEOUT_MS$1, JUDGE_BUDGET_USD$1, MAX_ATTEMPTS, MAX_PROMPT_BYTES, MAX_STREAM_BYTES, MAX_SOURCE_BYTES, MAX_UNTRACKED_FILES, ORIGIN_POLICY, SHA256_PATTERN, UUID_PATTERN$2, DIMENSIONS, JUDGE_SCHEMA$1, RUBRIC;
...
function resolveMaxRetainedObjectBytes(options = {}) {
	const configured = options.maxRetainedObjectBytes ?? process.env.CHD_EXPERIMENT_2702_MAX_RETAINED_OBJECT_BYTES ?? DEFAULT_MAX_RETAINED_OBJECT_BYTES;
	const parsed = typeof configured === "number" ? configured : Number(configured);
	if (!Number.isSafeInteger(parsed) || parsed <= 0 || parsed > DEFAULT_MAX_RETAINED_OBJECT_BYTES) fail$1(`max retained object bytes must be an integer from 1 through ${DEFAULT_MAX_RETAINED_OBJECT_BYTES}`);
	return parsed;
}
...
function addObject(objectMap, sourcePath, bytes, mediaType = "application/octet-stream") {
	if (bytes.length > objectMap.maxRetainedBytes - objectMap.retainedBytes) fail$1(`aggregate retained object byte budget exceeded while adding ${sourcePath}: ${objectMap.retainedBytes} retained + ${bytes.length} candidate > ${objectMap.maxRetainedBytes} byte budget`);
...
function verifyBundleDirectory(runtime, paths, marker, bundleDirectory, maxRetainedObjectBytes) {
	...
	for (const entry of manifest.artifacts) {
		...
		if (entry.sizeBytes > maxRetainedObjectBytes - plannedObjectBytes) fail$1(`aggregate retained object byte budget exceeded before reading sealed objects: ${plannedObjectBytes} planned + ${entry.sizeBytes} candidate > ${maxRetainedObjectBytes} byte budget`);
...
async function loadVerifiedTrial(optionsInput) {
	...
	const verified = verifyBundleDirectory(runtime, paths, marker, join(paths.sealRoot, marker.bundleDirectory), maxRetainedObjectBytes);
	return {
...
async function sealTrial(optionsInput, dependencies = productionValidators) {
	...
	const evidencePlan = assertNoUnknownEvidence(paths, expectedEvidencePaths(paths, armEvidence, selected)).map((path) => {
		const absolute = join(paths.trialRoot, path);
		const metadata = lstatSync(absolute);
		if (!metadata.isFile() || metadata.isSymbolicLink()) fail$1(`${path} is not a regular file`);
		if (metadata.size > MAX_OBJECT_BYTES) fail$1(`${path} exceeds its fixed size limit`);
		return {
			path,
			absolute,
	...
	const plannedEvidenceBytes = evidencePlan.reduce((total, entry) => total + entry.sizeBytes, 0);
	if (!Number.isSafeInteger(plannedEvidenceBytes) || plannedEvidenceBytes > maxRetainedObjectBytes) fail$1(`aggregate retained object byte budget exceeded before reading evidence bytes: ${plannedEvidenceBytes} bytes across ${evidencePlan.length} artifacts > ${maxRetainedObjectBytes} byte budget`);
	const objectMap = {
	...
	for (const { path, absolute } of evidencePlan) addObject(objectMap, path, readRegularBytes(absolute, MAX_OBJECT_BYTES, path), path.endsWith(".json") ? "application/json" : path.endsWith(".jsonl") ? "application/x-ndjson" : "application/octet-stream");
	...
	verifyBundleDirectory(runtime, paths, marker, bundleDirectory, maxRetainedObjectBytes);
...
var init_seal = __esmMin((() => {
	...
	DEFAULT_MAX_RETAINED_OBJECT_BYTES = 512 * 1024 * 1024;
