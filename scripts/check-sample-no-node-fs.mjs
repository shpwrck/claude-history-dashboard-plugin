#!/usr/bin/env node
// Public-sample node:fs-leak gate (#3628/#3735, follow-up to #3613).
//
// In the public sample `node:fs` has no implementation: Vite resolves it to an
...
// `check-bundle-size` only weighs bytes and the sample boundary only greps for
...
// sample-boundary job already built and finishes in milliseconds.
//
// Run it right after the sample build, against that build's dist/:
//   npm run build:sample && node scripts/check-sample-no-node-fs.mjs
//
// Flags:
...
import { inspectSampleBoundary } from './check-sample-boundary.mjs';
...
// shared module every artifact parser reads through; in the sample build it is
...
// into the public sample bundle entirely. Any future entry is a NEW #3613-class crash;
 ...
 * (scripts/check-sample-no-node-fs.test.mjs) can drive both a leaked and a clean
  ...
  for (const file of fsGraphChunks) {
    ...
    failures.push(
      `chunk "${file}" CONTAINS the node:fs graph (matched ${FS_GRAPH_MARKERS.join('/')}) — ` +
        `node:fs is an empty stub in the public sample, so evaluating this chunk throws ` +
...
function die(msg) {
  console.error(`\n✗ Public-sample node:fs gate ERROR — ${msg}\n`);
  process.exit(2);
}
...
function main() {
  ...
  try {
    ...
    die(`could not read ${assetsDir} — did \`npm run build:sample\` run first? (${err.message})`);
  }
  if (files.length === 0) {
  ...
  // Wrong-flavor preflight (#3650): a real public sample build cannot contain
  // SERVER_ONLY_MARKERS, so finding one
  ...
  // this catches running the sample gate against a server build.
  const boundary = inspectSampleBoundary(args.dist);
  const serverHit = boundary.offenders[0];
  if (serverHit) {
    die(
      `wrong-flavor dist: "${serverHit.file}" contains server-only marker "${serverHit.marker}", ` +
        `so ${args.dist} is a SERVER build, not a public sample build. Re-run \`npm run build:sample\` and ` +
        `point the gate at that dist.`,
    );
  }
  const { fsGraphChunks, reachers, failures } = evaluateFsLeak(files, contentOf);

  console.log(`\nSample node:fs gate — ${assetsDir} (${files.length} chunks scanned)`);
  ...
  if (failures.length > 0) {
    console.error('\n✗ Sample node:fs gate BLOCKED:');
    for (const f of failures) console.error(`  - ${f}`);
    console.error(
      '\nnode:fs resolves to an empty stub in the public sample, so any browser chunk that ' +
  ...
  console.log('\n✓ Sample node:fs gate PASSED: no browser chunk evaluates the node:fs graph.\n');
}
